...
@section('page-title', 'Supply Chain: Partner Refactoring')
...
<div class="page-fade-in max-w-4xl mx-auto pb-12">
    ...
    <div class="stagger-in mb-8">
        <div class="flex items-center space-x-3 text-slate-500 text-sm mb-2">
            <a href="{{ route('clinic.suppliers.index') }}" class="hover:text-emerald-600 transition-colors">Supplier Portfolio</a>
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
            <span class="text-slate-900 font-medium tracking-tight">Partner Node Refactoring</span>
        </div>
        <div class="flex items-center space-x-4">
            ...
            <div>
                <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Refactor Partner</h1>
                <p class="text-slate-500 font-medium italic">Adjusting node: {{ $supplier->name }}</p>
            </div>
        </div>
    </div>

    <form method="POST" action="{{ route('clinic.suppliers.update', $supplier) }}" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 text-sm">
                <div class="space-y-2 md:col-span-2">
                    ...
                    <input type="text" name="name" id="name" 
                        ...
                        placeholder="e.g. Global Medical Solutions" value="{{ old('name', $supplier->name) }}" required>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="contact_person" id="contact_person" 
                        class="w-full px-5 py-3.5 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-emerald-500 transition-all font-medium text-slate-700 placeholder-slate-400" 
                        placeholder="e.g. John Doe" value="{{ old('contact_person', $supplier->contact_person) }}">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="phone" id="phone" 
                        class="w-full px-5 py-3.5 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-emerald-500 transition-all font-medium text-slate-700 placeholder-slate-400" 
                        placeholder="+977-XXXXXXXXXX" value="{{ old('phone', $supplier->phone) }}">
            ...
            <div class="grid grid-cols-1 gap-8 text-sm">
                <div class="space-y-2">
                    ...
                    <input type="email" name="email" id="email" 
                        class="w-full px-5 py-3.5 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-emerald-500 transition-all font-medium text-slate-700 placeholder-slate-400" 
                        placeholder="orders@partner-network.com" value="{{ old('email', $supplier->email) }}">
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="address" id="address" rows="3"
                        class="w-full px-5 py-3.5 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-emerald-500 transition-all font-medium text-slate-700 placeholder-slate-400" 
                        placeholder="Complete headquarters address for transit logistics...">{{ old('address', $supplier->address) }}</textarea>
