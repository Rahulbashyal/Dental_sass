...
<div class="space-y-6">
    <div class="flex justify-between items-center">
        ...
        <div class="flex items-center space-x-2 bg-blue-50 px-4 py-2 rounded-lg border border-blue-200">
            ...
            <span class="text-sm font-medium text-blue-800">{{ now()->format('M Y') }}</span>
        </div>
    </div>
        ...
        <div class="stat-card group animate-card-hover">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold bg-gradient-to-r from-blue-600 to-sky-600 bg-clip-text text-transparent">NPR {{ number_format($stats['monthly_revenue'], 0) }}</p>
                    <p class="text-xs text-blue-600 mt-1">{{ $stats['monthly_revenue'] > 0 ? '↗ Growing' : 'Start earning' }}</p>
                </div>
        ...
        <div class="stat-card group">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold bg-gradient-to-r from-cyan-600 to-sky-600 bg-clip-text text-transparent">NPR {{ number_format($stats['pending_payments'], 0) }}</p>
                    <p class="text-xs text-cyan-600 mt-1">{{ $stats['pending_payments'] > 0 ? 'Needs collection' : 'All clear' }}</p>
                </div>
        ...
        <div class="stat-card group">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold bg-gradient-to-r from-indigo-600 to-blue-700 bg-clip-text text-transparent">{{ $stats['overdue_invoices'] }}</p>
                    <p class="text-xs text-indigo-600 mt-1">{{ $stats['overdue_invoices'] > 0 ? 'Urgent action' : 'All current' }}</p>
                </div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full 
                                    {{ $invoice->status === 'paid' ? 'bg-blue-100 text-blue-800' : 
                                       ($invoice->status === 'pending' ? 'bg-cyan-100 text-cyan-800' : 'bg-slate-100 text-slate-800') }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                ...
                                @if($invoice->status === 'pending')
                                    <form action="{{ route('clinic.invoices.mark-paid', $invoice) }}" method="POST" class="inline">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="text-sky-600 hover:text-sky-900">Mark Paid</button>
                                    </form>
                                @endif
                            </td>
                        </tr>
